...
| --- | --- | --- | --- |
| TD-001 | Dependent shapes | Broaden live replay beyond current refreshable subquery cases to nested, negated, and multi-hop dependency plans. | Protocol differential matrix includes representative nested/negated/multi-hop cases with matching output or deterministic must-refetch. |
| TD-002 | Shadow clients | Run unchanged compatible TypeScript clients against PulseSync in shadow mode. | Client traffic diff shows no correctness differences for covered production-like scenarios. |
