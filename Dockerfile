...
ENV ELECTRIC_STORAGE_MODE=memory
ENV ELECTRIC_STORAGE_DIR=/var/lib/pulsesync

EXPOSE 3000

HEALTHCHECK --interval=10s --timeout=3s --start-period=10s --retries=5 \
  CMD sh -ec 'wget -qO- "http://127.0.0.1:${ELECTRIC_PORT}/v1/health" | grep -q "\"status\":\"active\""'
