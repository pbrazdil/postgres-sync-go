...
check_docs() {
  ...
  grep -Fq 'v0.1.0-preview.3' CHANGELOG.md || fail "CHANGELOG.md must record the current preview release"
  grep -Fq 'v0.1.0-preview.3' README.md || fail "README.md must show the current preview release"
  grep -Fq 'v0.1.0-preview.3' pkg/pgsync/version.go || fail "pkg/pgsync version must match the current preview release"
